...
{
    private AudioClip clip;
    private const int maxConcurrentAudioSources = 513; // You can adjust this number for your stress test
    ...
    public IEnumerator TestConcurrentAudioPlayback()
    {
        ...
        for (int i = 0; i < maxConcurrentAudioSources; i++)
        {
            ...
            AudioSource audioSource = audioSourceObject.AddComponent<AudioSource>();
            audioSource.clip = clip;
            audioSource.volume = 1f; // Ensure volume is set to a reasonable level
            audioSource.Play();

            Debug.Log($"Playing audio source {i}: {audioSource.isPlaying}");

            // Wait a short duration before starting the next audio source
            yield return null; // This allows the next frame to start, avoiding performance hits
        }
        ...
        Assert.GreaterOrEqual(playingCount, maxConcurrentAudioSources - 5, 
            "Not enough audio sources were playing after playback started. Check if audio is being clamped.");
    }
}
*/